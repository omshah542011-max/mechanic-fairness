...
const DESCRIPTION_TIPS = [
  "Your car's make, model & year (e.g. 2019 Toyota Camry)",
  "What part was damaged or replaced (e.g. front brake pads)",
  "What the mechanic said was wrong",
  "Any extra charges they added on top"
];
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
      <div style={{ position: "relative", width: 200, height: 200 }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2 }}>
          ...
          <span style={{ fontSize: 10, color: "#777", letterSpacing: "0.1em", fontWeight: 700 }}>FAIRNESS</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ maxWidth: 500, margin: "0 auto", padding: "1.5rem 1rem 3rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.6rem" }}>
                  ...
                  <button onClick={() => setShowExamples(!showExamples)} style={{ background: "none", border: "none", color: "#5a5a9a", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>
                    {showExamples ? "Hide examples" : "See examples ↓"}
                  </button>
                </div>

                {/* Description tips */}
                <div style={{ background: "#0a0a14", border: "1px solid #1a1a2a", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
                  <p style={{ color: "#888", fontSize: 11, margin: "0 0 6px", letterSpacing: "0.06em", fontWeight: 700, textTransform: "uppercase" }}>Include these for best results:</p>
                  {DESCRIPTION_TIPS.map((tip, i) => (
                    <div key={i} className="tip-dot">
                      <span style={{ color: "#4a4a80", marginTop: 2, flexShrink: 0 }}>•</span>
                      <span>{tip}</span>
                    </div>
                  ))}
                </div>
                ...
                <textarea className="inp" rows={4} placeholder="e.g. 2019 Toyota Camry — front brake pads replaced, they also said I needed new rotors" value={description} onChange={e => setDescription(e.target.value)} style={{ resize: "vertical" }} />
              </div>
              <div>
              ...
              <div className={`drop ${dragOver ? "drag" : ""}`}
                ...
                {imagePreview
                  ...
                  : <div>
                      ...
                      <p style={{ color: "#888", margin: "0 0 4px", fontSize: 14 }}>Take a photo of your receipt and upload it here</p>
                      <p style={{ color: "#666", margin: 0, fontSize: 12 }}>or drag and drop · JPG, PNG supported</p>
                    </div>}
                <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => handleFile(e.target.files[0])} />
              </div>
              {imagePreview && <button onClick={() => { setImagePreview(null); setImageBase64(null); }} style={{ marginTop: 8, background: "none", border: "none", color: "#666", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Remove photo</button>}
            </div>
          )}
        ...
        {loading && (
          <div className="card" style={{ textAlign: "center", padding: "3rem 1rem" }}>
            ...
            <p style={{ color: "#888", fontSize: 14, margin: "0 0 4px", animation: "pulse 1.5s ease infinite" }}>Checking repair prices in your area...</p>
            <p style={{ color: "#555", fontSize: 12, margin: 0 }}>This usually takes 5–10 seconds</p>
          </div>
        )}
        ...
        {result && !loading && (
          <div ref={resultRef} style={{ animation: "fadeUp 0.5s ease forwards" }}>
            ...
            <div style={{ background: bgCol, border: `1.5px solid ${borderCol}`, borderRadius: 20, padding: "2.5rem 1.5rem 2rem", marginBottom: "1rem", textAlign: "center" }}>
              ...
              <p style={{ color: "#aaa", fontSize: 14, margin: "0 auto", maxWidth: 340, lineHeight: 1.6 }}>{result.summary}</p>
            </div>
            ...
            <div className="card">
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: result.overcharge_percentage > 0 ? "1rem" : 0 }}>
                <div style={{ flex: 1, background: "#08080f", borderRadius: 12, padding: "14px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#666", letterSpacing: "0.08em", marginBottom: 6, fontWeight: 700 }}>NORMAL PRICE</div>
                  ...
                  <div style={{ fontSize: 11, color: "#666", marginTop: 4 }}>what's typical in your area</div>
                </div>
                <div style={{ flex: 1, background: "#08080f", borderRadius: 12, padding: "14px", textAlign: "center" }}>
                  <div style={{ fontSize: 11, color: "#666", letterSpacing: "0.08em", marginBottom: 6, fontWeight: 700 }}>YOU PAID</div>
                  <div style={{ fontSize: 19, fontWeight: 800, color: hex }}>{result.currency_symbol}{result.what_they_paid}</div>
                  <div style={{ fontSize: 11, color: "#666", marginTop: 4 }}>what they charged you</div>
                </div>
              </div>
              ...
              {result.overcharge_percentage > 0 && (
                <div style={{ background: "#08080f", borderRadius: 12, padding: "14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                    <span style={{ fontSize: 13, color: "#777" }}>You overpaid by</span>
            ...
            <div className="card">
              ...
              <p style={{ color: "#ccc", fontSize: 15, lineHeight: 1.75, margin: 0 }}>{result.explanation}</p>
            </div>
            ...
            {result.tip && (
              <div style={{ background: "#031208", border: "1.5px solid #0a2a18", borderRadius: 16, padding: "1.4rem", marginBottom: "1rem" }}>
                ...
                <p style={{ color: "#6aaa84", fontSize: 14, lineHeight: 1.65, margin: 0 }}>{result.tip}</p>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: "1rem" }}>
              <button className="share-btn" style={{ flex: 1 }} onClick={() => {
                if (navigator.share) {
                  navigator.share({ title: "Mechanic Fairness Check", text: `My mechanic charged me ${result.currency_symbol}${result.what_they_paid}. Fairness score: ${result.fairness_score}%. Check yours!`, url: window.location.href });
                } else {
        ...
        <div style={{ background: "#0e0e18", border: "1.5px solid #1a1a28", borderRadius: 16, padding: "1.75rem 1.5rem", textAlign: "center", marginTop: "1rem" }}>
          ...
          <h3 style={{ color: "#d0d0f0", fontSize: 16, fontWeight: 700, margin: "0 0 8px" }}>This tool is completely free</h3>
          <p style={{ color: "#888", fontSize: 13, lineHeight: 1.65, margin: "0 auto 1.25rem", maxWidth: 300 }}>
            If we helped you avoid getting ripped off, consider buying us a coffee. It keeps the tool running for everyone.
          </p>
          <a
            href="https://www.paypal.com/paypalme/OmShah118"
            target="_blank"
          ...
          <p style={{ color: "#444", fontSize: 11, margin: "10px 0 0" }}>No pressure at all — using the tool is more than enough 🙏</p>
        </div>
